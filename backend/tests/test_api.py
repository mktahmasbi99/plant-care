...
def make_plant(client, **overrides):
    data = {"species": "Monstera deliciosa", "nickname": "Kitchen Monstera", "location": "Kitchen", "recommendation": {"interval_days": 4}}
...
def test_dashboard_derives_due_date_from_watering_without_creating_rounds(tmp_path, monkeypatch):
    ...
    assert dashboard["plants"][0]["status"] == {"status": "due", "next_check_date": "2026-09-22", "days_until_check": 0, "days_since": 4}


def test_not_watered_uses_selected_recheck_and_undo_restores_due_state(tmp_path, monkeypatch):
    client = client_for(tmp_path, monkeypatch)
    plant = make_plant(client, initial_last_watered="2026-09-18")
    check = client.post(f"/api/plants/{plant['id']}/checks", json={"outcome": "not_watered", "care_date": "2026-09-22", "next_check_date": "2026-09-25"})
    ...
    assert client.get("/api/dashboard?day=2026-09-22").json()["plants"][0]["status"]["status"] == "due"


def test_recheck_must_be_after_care_date(tmp_path, monkeypatch):
    client = client_for(tmp_path, monkeypatch)
    plant = make_plant(client)
    response = client.post(f"/api/plants/{plant['id']}/checks", json={"outcome": "not_watered", "care_date": "2026-09-22", "next_check_date": "2026-09-22"})
...
def test_order_is_persisted_and_requires_every_active_plant(tmp_path, monkeypatch):
    ...
    assert client.put("/api/plants/order", json={"plant_ids": [second["id"], first["id"]]}).status_code == 200
    assert [plant["id"] for plant in client.get("/api/dashboard").json()["plants"]] == [second["id"], first["id"]]
...
def test_v1_database_migrates_current_cadence_and_closes_open_rounds(tmp_path, monkeypatch):
    ...
    """)
    db.commit(); db.close()
    monkeypatch.setenv("PLANT_CARE_DB", str(path)); initialize()
    db = sqlite3.connect(path); db.row_factory = sqlite3.Row
    assert db.execute("SELECT sort_position FROM plants WHERE id=1").fetchone()[0] == 0
    assert tuple(db.execute("SELECT cadence_kind,cadence_value FROM watering_recommendations WHERE effective_to IS NULL").fetchone()) == ("days", 4)
    assert db.execute("SELECT status FROM inspection_rounds WHERE id=1").fetchone()[0] == "incomplete"
...
def test_backup_restore_keeps_valid_plant_data(tmp_path, monkeypatch):
    ...
    restored = client.post("/api/backups/restore-upload", data={"confirmation": "RESTORE"}, files={"file": ("backup.sqlite3", backup.content, "application/x-sqlite3")})
...
def test_photo_is_processed_and_can_become_a_cover(tmp_path, monkeypatch):
    client = client_for(tmp_path, monkeypatch); plant = make_plant(client); source = io.BytesIO(); Image.new("RGBA", (3200, 1200), (30, 160, 80, 140)).save(source, format="PNG")
    uploaded = client.post(f"/api/plants/{plant['id']}/photos", data={"caption": "New leaf"}, files={"file": ("leaf.png", source.getvalue(), "image/png")})
    assert uploaded.status_code == 201; photo_id = uploaded.json()["id"]
    assert client.post(f"/api/photos/{photo_id}/cover").status_code == 200
    assert client.get(f"/api/photos/{photo_id}?thumbnail=true").headers["content-type"] == "image/jpeg"
